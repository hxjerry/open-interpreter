...
def setup_text_llm(interpreter):
    ...
    def base_llm(messages):
        ...
        system_message += "\n\nTo execute code on the user's machine, write a markdown code block *with a language*, i.e ```java, ```python, ```shell, ```r, ```html, or ```javascript. You will recieve the code output."
